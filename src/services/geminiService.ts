...
For each stage, generate a JSON object with the following structure:
{
  ...
  "connectivity": [<blockIDs that come after this, representing dependencies. I want to specify that 1 can be connected to 2, 3, and then 2,3 can be connected to 4, then 4 can be connected to 5, and so on. But it's not like 1 can be connected with direct 7. It's to create a visual of tree going down. Don't force it tho, can even connect 3 sometimes..>]
}
...
User's query: "${query}"
Generate a long, detailed roadmap, till your context limit permits.
              `.trim()
            }
          ]
        }
